...
<div class="max-w-5xl mx-auto px-4 py-12">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-purple-600">Edit Pesanan</h1>
    ...
    <div class="bg-white rounded-xl shadow-lg p-8">
        <form action="{{ route('pesanan.update', $pesanan->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="meja_id" id="meja_id"
                    ...
                    @foreach($mejas as $m)
                        <option value="{{ $m->id }}" {{ $pesanan->meja_id == $m->id ? 'selected' : '' }}>
                            Meja {{ $m->nomor_meja }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="total_dp" id="total_dp"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500" required>
                    <option value="50000" {{ $pesanan->total_dp == 50000 ? 'selected' : '' }}>50.000</option>
                    <option value="100000" {{ $pesanan->total_dp == 100000 ? 'selected' : '' }}>100.000</option>
                    <option value="150000" {{ $pesanan->total_dp == 150000 ? 'selected' : '' }}>150.000</option>
                    <option value="200000" {{ $pesanan->total_dp == 200000 ? 'selected' : '' }}>200.000</option>
                </select>
            </div>
